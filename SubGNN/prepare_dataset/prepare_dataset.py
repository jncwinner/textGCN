...
def main():
    if config.GENERATE_SYNTHETIC_G: 
        ...
        nx.write_edgelist(synthetic_graph.graph, f"{config.DATASET_DIR}/edge_list.txt", data=False)
        sub_G = synthetic_graph.subgraphs
        sub_G_label = synthetic_graph.subgraph_labels
        mask = generate_mask(len(sub_G_label)) 
        write_f(f"{config.DATASET_DIR}/edge_list.txt", sub_G, sub_G_label, mask)
    if config.GENERATE_NODE_EMB: train_node_emb.generate_emb() 


if __name__ == "__main__":
    config.GENERATE_SYNTHETIC_G = False
    config.GENERATE_NODE_EMB = True
    config.DATASET_DIR = "C:/Users/jncwi/OneDrive/School/CS637/SubGNN/yahoo"
    main()
