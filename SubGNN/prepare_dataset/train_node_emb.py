...
# Global Variables
log_f = open(f"{config.DATASET_DIR}/node_emb.log", "w")
all_data = None 
device = None
best_val_acc = -1
...
def train(epoch, model, optimizer):
    ...
    if best_val_acc <= np.mean(acc_val) + eps:
        ...
        with open(f"{config.DATASET_DIR}/best_model.pth", 'wb') as f:
...
def test(model):
    ...
    model.load_state_dict(torch.load(f"{config.DATASET_DIR}/best_model.pth"))
    model.to(device)
    model.eval()
...
def generate_emb():
    ...
    all_data = preprocess.read_graphs(f"{config.DATASET_DIR}/edge_list.txt")
    ...
    # Save best embeddings
    torch.save(best_embeddings, f'{config.DATASET_DIR}/{config.CONV.lower()}_embeddings.pth')
